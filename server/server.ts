...
async function iterateFolderFiles(folderPath: string, fileSim: string, filePage: string, fileHITS: string) {
	...
	rows.forEach(async (row: string) => {
		...
		PageRankHash[vertex.trim()] = parseFloat(rank.trim());
	});
	...
	rowss.forEach(async (row: string) => {
		const [vertex, rank] = row.split(',');
		SimRankHash[vertex.trim()] = parseFloat(rank.trim());
	});
	...
	rowsss.forEach(async (row: string) => {
		const [vertex, rank] = row.split(',');
		HITSHash[vertex.trim()] = parseFloat(rank.trim());
	});
...
app.post('/search', (req: any, res: any) => {
	...
	for (const link of linkList) {
		const count = [...mainhashing[link].toLowerCase().matchAll(query.toLowerCase())].length;
		if (count > 0) {
			topTenSearches.push({ query, content: mainhashing[link], count, pageRank: pagehasing[link], simRank: simhasing[link], hits: hithasing[link] });
		}
	}
	...
	const results = topTenSearches.splice(10);
